...
mongoose
  ...
  .then(async () => {
    ...
    if (count === 0) {
      console.log('   ⚠️  Nenhuma subscription ativa. Isto é normal se ninguém se registou ainda.');
    }
    ...
    if (count > 0) {
      ...
      subs.forEach((sub, index) => {
        const lastUsed = sub.lastUsed 
          ...
          : 'Nunca';
        console.log(`   ${index + 1}. ${sub.userId?.name || 'Unknown'} (${sub.deviceType}) - Criada: ${new Date(sub.createdAt).toLocaleString('pt-PT')} - Última: ${lastUsed}`);
      });
    }

    // 6. Testar envio (opcional)
    console.log('\n📋 6. System ready para enviar notificações push!');
    console.log('   Quando um cliente faz uma reserva, o servidor enviará notificações para:');
    console.log(`   - Todos os ${count} dispositivos conectados`);
    console.log('   - Payload: { title, body, icon, badge, requireInteraction }');

    // 7. Detalhes de segurança
    console.log('\n🔐 Informações de Segurança:');
    console.log('   - VAPID keys estão carregadas corretamente');
    console.log('   - Subscriptions persistidas na MongoDB (não perdem com restarts)');
    console.log('   - Failed subscriptions são removidas automaticamente (410 errors)');
