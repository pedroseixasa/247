...
(function () {
  ...
  async function loadStaffMembers() {
    try {
      ...
      const staffSection = data?.staffSection || {};

      // Update section title and subtitle if provided
      const staffTitle = document.getElementById("staffTitle");
      const staffSubtitle = document.getElementById("staffSubtitle");
      if (staffSection.title && staffTitle) {
        staffTitle.textContent = staffSection.title;
      }
      if (staffSection.subtitle && staffSubtitle) {
        staffSubtitle.textContent = staffSection.subtitle;
      }

      const staffGrid = document.getElementById("staffGrid");
      if (!staffGrid || !barberCards) return;

      staffGrid.innerHTML = "";

      // Create cards for barber1 and barber2
      const barbers = [
        { prefix: "barber1", name: "Diogo Cunha" },
        { prefix: "barber2", name: "Ricardo Silva" },
      ];

      barbers.forEach((barber) => {
        const name = barberCards[`${barber.prefix}Name`] || barber.name;
        const role = barberCards[`${barber.prefix}Role`] || "Barbeiro";
        const description = barberCards[`${barber.prefix}Description`] || "";
        const image =
          barberCards[`${barber.prefix}Image`] || "images/cunha.png";
        const coverImage =
          barberCards[`${barber.prefix}CoverImage`] || "images/cunhacorte.png";
            ...
            <div class="staff-card-3d">
              <div class="staff-card">
                <div class="staff-card-wrapper">
                  <img src="${coverImage}" class="staff-cover-image" alt="${name}">
                </div>
                <img src="${image}" class="staff-character-image" alt="${name}">
              </div>
            </div>
  ...
  function initStaff3D() {
    ...
    staffCards.forEach((card) => {
      let isHovered = false;
      const cardWrapper = card.querySelector(".staff-card-wrapper");

      // Activate 3D on hover/touch
      ...
      if (cardWrapper) {
        card.addEventListener("mousemove", (e) => {
          ...
          cardWrapper.style.transform = `perspective(900px) rotateX(${20 + rotateX}deg) rotateY(${rotateY}deg) translateZ(0)`;
        });

        card.addEventListener("mouseleave", () => {
          cardWrapper.style.transform =
            "perspective(900px) translateY(-3%) rotateX(20deg) translateZ(0)";
        });
      }
    });
  }
  ...
  // Also call when site settings are updated
  const originalLoadSiteSettings = window.loadSiteSettings;
  if (originalLoadSiteSettings) {
    window.loadSiteSettings = function () {
      originalLoadSiteSettings.call(this);
      loadStaffMembers();
    };
  }
})();
